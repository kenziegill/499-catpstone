...
- **No PII de-identification.** Research reports in production deployments routinely contain quasi-identifying combinations (role + company size + location) that simple PII scrubbing misses. The proposal sketched a two-pass approach: regex/NER for obvious PII followed by a Haiku classification pass to flag quasi-identifiers for human review before indexing. This is out of scope for the PoC because it does not affect demonstration of the core architecture (agent control flow, structural anti-hallucination, RAG retrieval). It is the most important step before this system could index real research reports under NDA.
